...
const TableComponent = (props) => {
  ...
  datasarr.forEach((v, i) => {
    datas.set(i.toString(), v);
  });

  const keys = datas.size > 0 ? Object.keys(datas.get("0")) : false;

  if (!keys) {
    return <h1 className="align-middle">No Data ToBe Shown</h1>;
  }
  ...
  const handleInit = () => {
    const res = [];
    const checkbox = document.querySelectorAll('input[name="isSelected"]:checked')
    checkbox.forEach((v) => {
        console.log(v.value)
        res.push(v.value);
      });
